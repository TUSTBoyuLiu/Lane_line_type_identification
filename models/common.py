...
class BiFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 第一层
        p7_td = self.p7_td[0](p7_x)  # 首先从p7_x开始，使用1x1卷积调整通道数
        p6_td = self.p6_td[0](p6_x)  # 然后将调整后的结果上采样两倍，与p6_x进行融合
        p6_td = F.interpolate(p7_td, size=p6_x.shape[-2:], mode='nearest') + p6_td  # 相加后使用3x3卷积进行特征融合
        p6_out = self.p6_out[0](p6_td)  # 调整通道数
        p6_out = self.p6_out_gam[0](p6_out)  # 添加GAM注意力机制
        p5_td = self.p5_td[0](p5_x)  # 对p5_x进行1x1卷积调整通道数，再将调整后的结果上采样两倍，与p6_out进行融合
        p5_td = F.interpolate(p6_out, size=p5_x.shape[-2:], mode='nearest') + p5_td  # 相加后使用3x3卷积进行特征融合
        p5_out = self.p5_out0  # 调整通道数
        p5_out = self.p5_out_gam0  # 添加GAM注意力机制

        # 第二层
        p5_td = self.p5_td[1](p5_out)  # 从p5_out开始，使用1x1卷积调整通道数
        p4_td = self.p4_td[0](p4_x)  # 对p4_x进行1x1卷积调整通道数，再将调整后的结果上采样两倍，与p5_td进行融合
        p4_td = F.interpolate(p5_td, size=p4_x.shape[-2:], mode='nearest') + p4_td  # 相加后使用3x3卷积进行特征融合
        p4_out = self.p4_out[0](p4_td)  # 调整通道数
        p4_out = self.p4_out_gam[0](p4_out)  # 添加GAM注意力机制

        # 第三层
        p4_td = self.p4_td[1](p4_out)  # 从p4_out开始，使用1x1卷积调整通道数
        p3_td = self.p3_td[0](p3_x)  # 对p3_x进行1x1卷积调整通道数，再将调整后的结果上采样两倍，与p4_td进行融合
        p3_td = F.interpolate(p4_td, size=p3_x.shape[-2:], mode='nearest') + p3_td  # 相加后使用3x3卷积进行特征融合
        p3_out = self.p3_out[0](p3_td)  # 调整通道数
        p3_out = self.p3_out_gam[0](p3_out)  # 添加GAM注意力机制

        # 第四层
        p4_td = self.p4_td[2](p4_out)  # 从p4_out开始，使用1x1卷积调整通道数，然后与p5_td融合
        p4_td = F.interpolate(p5_td, size=p4_out.shape[-2:], mode='nearest') + p4_td  # 相加后使用3x3卷积进行特征融合
        p4_out = self.p4_out[1](p4_td)  # 调整通道数
        p4_out = self.p4_out_gam[1](p4_out)  # 添加GAM注意力机制

        # 第五层
        p5_td = self.p5_td[2](p5_out)  # 从p5_out开始，使用1x1卷积调整通道数，然后与p6_td融合
        p5_td = F.interpolate(p6_td, size=p5_out.shape[-2:], mode='nearest') + p5_td # 相加后使用3x3卷积进行特征融合
        p5_out = self.p5_out1  # 调整通道数
        p5_out = self.p5_out_gam1  # 添加GAM注意力机制

        # 第六层
        p6_td = self.p6_td[0](p6_x)  # 对p6_x进行1x1卷积调整通道数，再将调整后的结果上采样两倍，与p5_td进行融合
        p6_td = F.interpolate(p5_td, size=p6_x.shape[-2:], mode='nearest') + p6_td  # 相加后使用3x3卷积进行特征融合
        p6_out = self.p6_out[0](p6_td)  # 调整通道数
        p6_out = self.p6_out_gam[0](p6_out)  # 添加GAM注意力机制

        # 第七层
        p6_td = self.p6_td[1](p6_out)  # 从p6_out开始，使用1x1卷积调整通道数，然后与p7_td融合
        p7_td = self.p7_td[0](p7_x)  # 对p7_x进行1x1卷积调整通道数
        p7_td = F.interpolate(p6_td, size=p7_x.shape[-2:], mode='nearest') + p7_td  # 相加后使用3x3卷积进行特征融合
        p7_out = self.p7_out[0](p7_td)  # 调整通道数
        p7_out = self.p7_out_gam[0](p7_out)  # 添加GAM注意力机制

        # 第八层
        p7_td = self.p7_td[1](p7_out)  # 从p7_out开始，使用1x1卷积调整通道数，然后与p6_td融合
        p6_td = F.interpolate(p7_td, size=p6_out.shape[-2:], mode='nearest') + p6_td  # 相加后使用3x3卷积进行特征融合
        p6_out = self.p6_out[1](p6_td)  # 调整通道数
        p6_out = self.p6_out_gam[1](p6_out)  # 添加GAM注意力机制

        return [p3_out, p4_out, p5_out, p6_out, p7_out]  # 返回多个层级的特征图
# ------------------------------ BiFPN end ----------------------------------------
